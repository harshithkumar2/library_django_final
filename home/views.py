...
def forgot_data(request):
    if request.method == 'POST':
        uname = request.POST['uname']
        mail = request.POST['mail']
        pasw = request.POST['pass']
        rpasw = request.POST['rpass']
        if pasw == rpasw:
            data = User.objects.get(username=uname)
            if data.email == mail:
                data.set_password(pasw)
                data.save()
                messages.success(request, 'Password changed')
                return redirect('login')
            else:
                messages.info(request, 'Mail din match for user')
                return redirect('forgotpass')
        else:
            messages.error(request, 'Password din match')
            return redirect('forgotpass')
